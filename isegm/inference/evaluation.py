...
def evaluate_dataset(dataset, predictor, max_clicks, vis=True, vis_path='./experiments/vis_val/', **kwargs):
    ...
    for index in tqdm(range(len(dataset)), leave=False):
        ...
        _, sample_ious, _ = evaluate_sample(sample.image, sample.gt_mask, sample.init_mask, predictor, metric='iou',
                                            sample_id=index, max_clicks=max_clicks, vis=vis, save_dir=save_dir,
                                            index=index, **kwargs)
        _, sample_dices, _ = evaluate_sample(sample.image, sample.gt_mask, sample.init_mask, predictor, metric='dice',
                                             sample_id=index, max_clicks=max_clicks, vis=vis, save_dir=save_dir,
                                             index=index, **kwargs)
...
def evaluate_sample(image, gt_mask, init_mask, predictor, max_metric_thr, metric,
    ...
    with torch.no_grad():
        ...
        if vis and sample_id % 1000 == 0:
            if predictor.focus_roi is not None:
                ...
                out_image, focus_image = vis_result_refine(image, pred_mask, gt_mask, init_mask, metric_value,
                                                           click_indx + 1,
                ...
                clicks_list = clicker.get_clicks()
                last_y, last_x = predictor.last_y, predictor.last_x
                out_image = vis_result_base(image, pred_mask, gt_mask, init_mask, metric_value, click_indx + 1,
                                            clicks_list,
